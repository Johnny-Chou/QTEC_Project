...
//    public static final int BASE_SCREEN_HEIGHT = 1920;
    public static final float BASE_SCREEN_WIDTH_FLOAT = 375F;
    public static final float BASE_SCREEN_HEIGHT_FLOAT = 667F;
    ...
    private static void scaleViewGroup(ViewGroup viewGroup) {
        ...
        for (int i = 0; i < viewGroup.getChildCount(); ++i) {
            ...
            if (view instanceof Toolbar){
                for (int j = 0; j < ((Toolbar)view).getChildCount(); ++j) {
                    View child = ((Toolbar)view).getChildAt(j);
                    if (child instanceof TextView){
                        scaleView(child);
                    }
                }
            }
            if (view instanceof ViewGroup && !(view instanceof Toolbar)) {
                scaleViewGroup((ViewGroup) view);
            }
            scaleView(view);
        }
